...
// Register the widget
add_action( 'widgets_init', create_function( '', 'return register_widget("liroom_Widget_block5");'));

// The widget class
class liroom_Widget_block5 extends WP_Widget {

	public function __construct() {
		$widget_ops = array('classname' => 'block5_widget', 'description' => esc_html__( "Display related posts.", 'liroom-lite') );
		parent::__construct('ft_block5', esc_html__('FT Block 5 - Recent Posts', 'liroom-lite'), $widget_ops);
		$this->alt_option_name = 'widget_block5';
	...
	public function widget( $args, $instance ) {
		...
		$cache = array();
		if ( ! $this->is_preview() ) {
			$cache = wp_cache_get( 'widget_block5', 'widget' );
		}
		if ( ! is_array( $cache ) ) {
			$cache = array();
		}
		...
		$categories[0] = $featured_categories[0];
		...
		$custom_query = new WP_Query( apply_filters( 'widget_block5_posts_args', $custom_query_args ) );
		$count        = 0;
		if ( $custom_query->have_posts() ) :
			...
			break;
			case('line'):
				echo '<div class="block5 block5_grid">';
				echo '<div class="row">';
					while ( $custom_query->have_posts() ) : $custom_query->the_post();
					$count++;
					?>
					<article <?php post_class( 'col-md-12 col-sm-12' ); ?>>
					    <div class="entry-thumb">
					        <a href="<?php echo esc_url( get_permalink() ); ?>" title="<?php the_title(); ?>">
								<?php
								if ( has_post_thumbnail( ) ) {
			...
			break;
			default:
				echo '<div class="block5 block5_list">';
					while ( $custom_query->have_posts() ) : $custom_query->the_post();
		...
		if ( ! $this->is_preview() ) {
			...
			wp_cache_set( 'widget_block5', $cache, 'widget' );
		} else {
			ob_end_flush();
		}
	}
	...
	public function remove_cache() {
		wp_cache_delete('widget_block5', 'widget');
	}
	...
	public function form( $instance ) {
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'layout' ); ?>"><?php esc_html_e('Posts block layout:', 'liroom-lite') ?></label>
