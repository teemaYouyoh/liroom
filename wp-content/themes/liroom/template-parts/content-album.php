<script type="text/javascript">
   admixerML.fn.push(function () {


       admixerML.defineSlot({
           z: 'ae0f3c68-ec4b-4a6d-97d4-1ce2c14b7ea7',
           ph: 'admixer_ae0f3c68ec4b4a6d97d41ce2c14b7ea7_zone_12255_sect_3677_site_3331',
           i: 'inv-nets'
       });
       admixerML.singleRequest();
   });
</script>
<?php
/**
 * Template part for displaying single posts.
 *
 * @link https://codex.wordpress.org/Template_Hierarchy
 *
 * @package liroom
 */
 
$actual_link = (isset($_SERVER['HTTPS']) ? "https" : "http") . "://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]";
$urlCur = str_replace( 'local','com.ua',$actual_link );
$liroom_album_id = get_the_ID() ; 
$liroom_album_year = get_post_meta( $liroom_album_id, 'liroom_album_year', true ) ; 
		$curr_year = date("Y");
		if ($liroom_album_year == '') $liroom_album_year = $curr_year;
		
$liroom_album_month = esc_attr( get_post_meta( $liroom_album_id, 'liroom_album_month', true ) ); 
		if ($liroom_album_month == '') $liroom_album_month = 0;
$months = array( 0 => '', 1 => 'январь', 2 => 'февраль', 3 => 'март', 4 => 'апрель', 5 => 'май', 6 => 'июнь', 7 => 'июль', 8 => 'август', 9 => 'сентябрь', 10 => 'октябрь', 11 => 'ноябрь', 12 => 'декабрь' );

$liroom_album_embed =  get_post_meta( $liroom_album_id, 'liroom_album_video', true ) ; 
$liroom_album_links =  get_post_meta( $liroom_album_id, 'liroom_album_links', true ) ; 
$liroom_album_link_review =  get_post_meta( $liroom_album_id, 'liroom_album_link_review', true ) ; 
$liroom_album_soundcloud =  get_post_meta( $liroom_album_id, 'liroom_album_sound', true ) ; 
$liroom_album_play =  get_post_meta( $liroom_album_id, 'liroom_album_play', true ) ; 
$liroom_album_itunes =  get_post_meta( $liroom_album_id, 'liroom_album_itunes', true ) ; 
$liroom_album_bandcamp =  get_post_meta( $liroom_album_id, 'liroom_album_bandcamp', true ) ; 
$liroom_album_format =  get_post_meta( $liroom_album_id, 'liroom_album_format', true ) ; 
$format = "LP";
if($liroom_album_format == 2) $format = "EP";
	
$grId = get_post_meta( $liroom_album_id, 'liroom_album_group', true );
$gr = get_post( $grId );
$grTitle = $gr->post_title;
$liroom_album_group_about = $gr->post_content;
$liroom_album_group_url = get_post_permalink( $grId ); 
$liroom_album_group_img = get_the_post_thumbnail( $grId, 'thumbnail', array('class' => 'album_thumb_left')); 
if ($liroom_album_group_img == '') {
	$liroom_album_group_img = '<img src="http://placehold.it/150x150" alt="" class="album_thumb_left attachment-album_small_thumb size-album_small_thumb wp-post-image"/>';
}
//
$tags = get_terms( 'tag_category', array( 
   'number' => 999, 
   'orderby' => 'count', 
   'order' => 'DESC', 
   'hide_empty' => false,
   ) );
$band_tags = get_the_term_list($liroom_band_id, 'album_tags', '<li>', '</li><li>', '</li>');
$band_labels = get_the_term_list($liroom_band_id, 'album_labels', '<li>', '</li><li>', '</li>');





$args = array(
	'meta_key'    => 'liroom_song_album',
	'meta_value'  => $liroom_album_id,
	'post_type'   => 'songs',
	'orderby'   => 'meta_value',
	'order'   => 'DESC'
	
	
);

$songs = get_posts( $args );



?>

<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	<header class="entry-header entry-header-single">
		<?php the_title( '<h1 class="entry-title">Альбом: ', '</h1>' ); ?>
		<span class="format">(<?php echo $format; ?>)</span>
		<?php liroom_meta_1(true); ?>
	</header><!-- .entry-header -->

	<div class="entry-content">

	
        <figure class="image-overlay">
          <?php
          if( function_exists( 'has_post_video' ) && has_post_video() ){
            the_post_video( "large" );
          }elseif ( has_post_thumbnail() ) {
            the_post_thumbnail( "large" );
          }
          ?>
        </figure>
		
		<div class="social-share top">
			<div class="fb-like" data-href="<?php echo $urlCur; ?>" data-layout="button_count" data-action="like" data-size="small" data-show-faces="false" data-share="false"></div>
			<a href="https://twitter.com/share?ref_src=twsrc%5Etfw" class="twitter-share-button" data-show-count="false">Tweet</a><script async src="https://platform.twitter.com/widgets.js" charset="utf-8"></script>
			<g:plusone action="share" href="<?php echo $urlCur; ?>" annotation="bubble" size="tall"></g:plusone>
		</div>
		
	<?php if( current_user_can('administrator') && !empty($songs) ) {  ?>
		<div class="content-wrapper clearfix">
			<div class="clearfix"><b>Тексти пісень альбому: </b></div>
			<ul style="margin: 20px 0 0">
			<?php foreach($songs as $post){
					setup_postdata($post);
					echo '<li><a href="'.get_the_permalink().'" >'.get_the_title().'</a></li>';
				  };
				  wp_reset_postdata();?>
			</ul>
		</div>
		
	<hr style="margin: 20px 0" />
	<?php } ?>
  <div class="content-wrapper clearfix"><?php the_content( );?></div>
		
  <div class="divider-wrapper clearfix">
	<div class="clearfix"><b>Дата виходу: </b><?php echo $months[intval($liroom_album_month)];?> <?php echo $liroom_album_year;?></div>
	<?php if( $liroom_album_link_review != '' ) { 
	$ptitle = get_the_title($liroom_album_link_review);
	$ppermalink = get_permalink($liroom_album_link_review);
    ?>
    <div><span>Рецензія: <a href="<? echo $ppermalink;?>" title="<? echo $ptitle;?>"><? echo $ptitle;?></a></span></div><?php } ?>
   </div>
   
   <?php if($band_tags){ ?>
   <div class="dt_post_content margin0">
   <div class="tags-wrapper">
   <ul class="tags-widget clearfix">
   <li class="trending">Жанри:</li>
   <?php echo $band_tags; ?>    
   </ul></div></div>
	   <?php } ?>
   
   
   <?php if($band_labels){ ?>
   <div class="dt_post_content">
   <div class="tags-wrapper">
   <ul class="tags-widget clearfix">
   <li class="trending">Лейбли:</li>
   <?php echo $band_labels; ?>    
   </ul></div></div>
	   <?php } ?>
   
   
    <div class="box-wrapper clearfix">
		<h2 class="block-title"><span>Купити музику:</span></h2>
		  <div class="band-contact"> 			
			<?php if($liroom_album_play != ''){ ?><a href="<?php echo $liroom_album_play;?>" class="play" target="_blank"><img src="data:image/svg+xml;utf8;base64," /></a><?php } ?>
			<?php if($liroom_album_itunes != ''){ ?><a href="<?php echo $liroom_album_itunes;?>" class="itunes" target="_blank"><img src="data:image/svg+xml;utf8;base64," /></a><?php } ?>
			<?php if($liroom_album_bandcamp != ''){ ?><a href="<?php echo $liroom_album_bandcamp;?>" class="itunes" target="_blank"><img src="data:image/svg+xml;utf8;base64," /></a><?php } ?>
		  </div>
	</div>
	  
	<?php if( $liroom_album_soundcloud != '' ) { ?> 
    <div class="box-wrapper clearfix">
		<h2 class="block-title"><span>Soundcloud:</span></h2>
		<div class="clearfix"><?php echo $liroom_album_soundcloud;?></div>
	</div>
	<?php } ?>
	<?php if( $liroom_album_embed != '' ) { ?> 
    <div class="box-wrapper clearfix">
		<h2 class="block-title"><span>Відео:</span></h2>
		<div class="clearfix"><iframe width="560" height="315" src="https://www.youtube.com/embed/<?php echo $liroom_album_embed;?>" frameborder="0" allowfullscreen></iframe></div>
	</div>
	<?php } ?>
   
   
	<?php if( $liroom_album_links != '' ) { ?> 
    <div class="box-wrapper clearfix">
		<h2 class="block-title"><span>Придбати:</span></h2>
		<div class="clearfix"><?php echo $liroom_album_links;?></div>
	</div>
	<?php } ?>
	
   <h2 class="block-title"><span>Група: <a class="url fn n" href="<?php echo $liroom_album_group_url;?>"><?php echo $gr->post_title;?></a></span></h2>
    <div class="row">
		<div class="col-md-12 group h4">
			<?php if( $liroom_album_group_img != '' ) { ?> 
				<a class="url fn n" href="<?php echo $liroom_album_group_url;?>"><?php echo $liroom_album_group_img;?></a>
			<?php } ?>
			<?php if( $gr->post_content != '' ) { ?> 
				<p style="margin-top:20px"><?php echo $gr->post_content;?></p>
			<?php } ?>
		</div>
		
	</div>
		<?php 			
				global $post;
				$args = array( 'numberposts' => -1, 'post_type' => 'albums' );
				$posts = get_posts( $args );					
				$myquery = get_posts( "post_type=albums&meta_key=liroom_album_group&meta_value=$grId&order=ASC" );
				 if (count($myquery) > 1){
					?>					
					<div class="box-wrapper clearfix">
					<h2 class="block-title"><span>Інші альбоми групи</span></h2>	
					<div class="row main-block">
					<?php
					foreach( $myquery as $post ): 
						if ($post->ID != $liroom_album_id) get_template_part( 'template-parts/block', 'album-rel' );
					endforeach;
					?>					
					</div>
					</div>
					<?php					 
				 }
				wp_reset_postdata(); // сброс
		?>
		
	<?php echo do_shortcode('[mistape]'); ?>
	</div><!-- .entry-content -->
</article><!-- #post-## -->
