	...
	class TP_Request {
		...
		public function vote() {
			...
			if ( $this->poll !== false ): // Check poll
				...
				$fields_errors      = apply_filters( 'totalpoll/filters/poll/vote/fields', $fields->run() );
				$limitations_errors = apply_filters( 'totalpoll/filters/poll/vote/limitations', $this->poll->limitations()->run() );
				...
				if ( empty( $limitations_errors ) === true && empty( $fields_errors ) === true && $this->poll->vote( $this->choices ) === true ):
					...
					if ( is_user_logged_in() ):
						...
						if ( $this->poll->settings( 'logs', 'details', 'user_login', 'enabled' ) ):
							$this->log['details']['user_email'] = 'Username: ' . wp_get_current_user()->user_login;
						endif;
