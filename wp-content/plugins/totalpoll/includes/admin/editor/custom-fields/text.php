						...
						<div class="settings-field">
							<label>
								<input
									...
									data-rename="totalpoll[settings][fields][{{new-index}}][validations][unique][enabled]"
									<?php checked( empty( $custom_field['validations']['unique']['enabled'] ), false ); ?>
									>
								<?php _e( 'Unique', TP_TD ); ?>
								<span class="totalpoll-feature-details" title="<?php esc_attr_e( 'This field value must be unique in submissions table.', TP_TD ); ?>">?</span>
							</label>
						</div>
